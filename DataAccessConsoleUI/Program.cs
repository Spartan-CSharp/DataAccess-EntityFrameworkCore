...
namespace DataAccessConsoleUI
{
	internal static class Program
	{
		...
		{
			_data = new DataLogic(_configuration, DBTYPES.EntityFramework);
		}

		private static void ProgramIntro()
		{
			Console.WriteLine();
			Console.WriteLine("Welcome to Entity Framework CRUD!");
					...
					case 'c':
						...
						switch ( recordtype )
						{
							...
							case 'a':
								...
								break;
							case 'e':
								Employer employer = CreateNewEmployer();
					...
					case 'r':
						...
						switch ( recordtype )
						{
							...
							case 'a':
								...
								break;
							case 'e':
								Employer employer = RetrieveEmployer();
								if ( employer != null )
								{
									DisplayEmployer(employer);
								}
		...
		private static Person CreateNewPerson()
		{
			...
			if ( selection != 1 )
			{
				...
				do
				{
					...
					{
						Address address = RetrieveAddress();
						Address checkifexistingaddress = output.Addresses.Where(x => x.Id == address.Id).FirstOrDefault();
		...
		private static Person RetrievePerson()
		{
			bool selectionvalid = false;
			int personid = 0;
			Person output = new Person();

			List<Person> people = _data.GetAllPeople();
		...
		private static Address RetrieveAddress()
		{
			bool selectionvalid = false;
			int addressid = 0;
			Address output = new Address();

			List<Address> addresses = _data.GetAllAddresses();
		...
		private static Employer RetrieveEmployer()
		{
			bool selectionvalid = false;
			int employerid = 0;
			Employer output = new Employer();

			List<Employer> employers = _data.GetAllEmployers();
		...
		private static void UpdatePerson(this Person person)
		{
			...
			do
			{
				...
				if ( selection == 1 )
				{
					if ( person.Addresses.Count > 0 )
					{
						...
						Address address = person.Addresses.Where(x => x.Id == addressselect).FirstOrDefault();
				...
				else if ( selection == 2 )
				{
					...
					{
						Address address = RetrieveAddress();
						Address checkifexistingaddress = person.Addresses.Where(x => x.Id == address.Id).FirstOrDefault();
						if ( checkifexistingaddress == null )
						{
							person.Addresses.Add(address);
						}
		...
		private static void UpdateAddress(this Address address)
		{
		...
		private static void UpdateEmployer(this Employer employer)
		{
			...
			else
			{
				...
				foreach ( Address address in employer.Addresses )
				{
					Console.WriteLine($"{address.Id}: {address.StreetAddress}, {address.City}, {address.State}  {address.ZipCode}");
				}
			}
			...
			do
			{
				...
				if ( selection == 1 )
				{
					if ( employer.Addresses.Count > 0 )
					{
						Console.Write("Please select the address to remove using the number: ");
						addressvalid = int.TryParse(Console.ReadLine(), out addressselect);
						Address address = employer.Addresses.Where(x => x.Id == addressselect).FirstOrDefault();
						if ( addressvalid && address != null )
						{
				...
				else if ( selection == 2 )
				{
					...
					{
						Address address = RetrieveAddress();
						Address checkifexistingaddress = employer.Addresses.Where(x => x.Id == address.Id).FirstOrDefault();
						if ( checkifexistingaddress == null )
						{
							employer.Addresses.Add(address);
						}
		...
		{
			Address output = RetrieveAddress();
		...
		{
			Employer output = RetrieveEmployer();
		...
		private static void DisplayPerson(Person person)
		{
			...
			if ( person.Employer != null )
			{
				...
				if ( person.Employer.Addresses.Count > 0 )
				{
					Console.WriteLine();
					Console.WriteLine("Addresses for the Employer:");
					foreach ( Address address in person.Employer.Addresses )
					{
						Console.WriteLine($"{address.StreetAddress}, {address.City}, {address.State}  {address.ZipCode}");
					}
				}
		...
		{
			Console.WriteLine();
			Console.WriteLine("Thank you for using Entity Framework CRUD!");
			Console.WriteLine();
		}
	}
}
